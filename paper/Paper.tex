...
% TODO: bettr defn
\begin{equation}
  \digSumPeriodic = \sum_{i = i_0}^{i_0 + \rationalPeriod} \mathrm{digit}(n, d, b, i)
\end{equation}
...
\begin{align*}
  ...
  &= \left(\theta_{i} + \frac{\rationalPeriodTotal}{\rationalPeriod} \digSumPeriodic\right) \mod b\\
  &= \theta_{i} \mod b + \frac{\rationalPeriodTotal}{\rationalPeriod} \digSumPeriodic \mod b.
\end{align*}
So, after $p$ periods of length $\rationalPeriodTotal$ where $p \cdot \frac{\rationalPeriodTotal}{\rationalPeriod}  \cdot \digSumPeriodic \equiv 0 \mod b$,
$$
...
First, let $A_d = \set{j \mid j \in [k] \and \inclIndic{d}{j} = 1}$
...
We can then see that for $(n, b, d, \theta_i) \in \pmb{s}_i$
\begin{align*}
  \seedAngle{x \rationalPeriodTotal + y} &= \sum_{i = 0}^{x \rationalPeriodTotal + y} \mathrm{digit}(n, b, d, i) \\
  &= \sum_{p = 0}^{x\periodFrac - 1} \sum_{q=0}^{\rationalPeriod - 1} \mathrm{digit}(n, b, d, p\rationalPeriodTotal + b)
    + \sum_{p = 0}  \\
    % ARGS::....
  &= x\cdot \periodFracRational \cdot \digSumPeriodic + .
\end{align*}
